...
namespace BTF
{
    public class awkParser : InterPreter
    {
        private int ptrsize;
        ...
        public awkParser(string code, int ptrsize) : base(code, ptrsize)
        {
        ...
        protected override void Action(Opcode command)
        {
            ...
            else if (command == Opcode.Output)
            {
                ...
                output += $"{들여쓰기}printf(\"%c\",ptr[memory]);\n";
            }
            ...
            else if (command == Opcode.Input)
            {
                ...
                output += $@"{들여쓰기} getline ptr[memory];{Environment.NewLine}";
            }
            if (command == Opcode.Closeloop)
            {
                ...
                if (nested == 1)
                {
                    output += $"{들여쓰기}}}{Environment.NewLine}";
                    들여쓰기 = "                     " + last들여쓰기;
                }
                else if (nested == 0)
                {
